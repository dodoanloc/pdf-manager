...
UNIT_CODES = {
    'Hội sở': 'HS',
    'PGD Xuân Lai': 'XL',
    # Legacy/default code used by existing seeded users and create APIs.
    'THX': 'THX'
}
...
def verify_password(input_password: str, stored_hash: str) -> bool:
    # Backward-compatible hook: current seeded users use plain sha256 hex.
    return bool(stored_hash) and stored_hash == hash_password(input_password or '')
...
def is_cam_co_location(value: str) -> bool:
    normalized = str(value or '').strip().lower()
    normalized = normalized.replace('ầ', 'â').replace('ố', 'ô')
    return 'cầm cố' in str(value or '').lower() or 'cam co' in normalized
...
def change_password():
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT password_hash FROM users WHERE username = ?', (session.get('user'),))
    row = c.fetchone()
    if not row or not verify_password(current_password, row[0]):
        conn.close()
        return jsonify({'error': 'Mật khẩu hiện tại không đúng'}), 400
    c.execute('UPDATE users SET password_hash = ? WHERE username = ?', (hash_password(new_password), session.get('user')))
    conn.commit()
    conn.close()
    logging.info(f"Password changed by {session.get('user')}")
...
def create_user():
    data = request.get_json()
    ok, msg = validate_password(data.get('password') or '')
    if not ok:
        return jsonify({'error': msg}), 400
    conn = get_db()
    c = conn.cursor()
